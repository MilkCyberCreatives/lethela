...
export async function getSqliteReadinessCounts(): Promise<SqliteCounts | null> {
  ...
  try {
    ...
    return {
      activeVendors: count(
        db,
        "SELECT COUNT(*) AS n FROM Vendor WHERE isActive = 1 AND status = 'ACTIVE'",
      ),
      activeProducts: count(
        ...
        `SELECT COUNT(*) AS n
         ...
         WHERE p.inStock = 1 AND v.isActive = 1 AND v.status = 'ACTIVE'`,
      ),
      pendingVendors: count(db, "SELECT COUNT(*) AS n FROM Vendor WHERE status = 'PENDING'"),
      pendingRiders: count(
        db,
