...
export async function POST() {
  try {
    ...
    const priceMsg = [
      {
        role: "system",
        content:
          "You are a pricing analyst. Given product names and prices (ZAR), suggest up to 5 price tweaks (+/-) to improve conversion. Keep each suggestion short.",
      },
      {
        role: "user",
        content:
          vendor.products
            .map((p: any) => `${p.name} : R${(p.priceCents / 100).toFixed(0)}`)
            .join("\n") || "No products",
      },
    ] as any;
    ...
    const lowStock = vendor.products
      .filter((p: any) => /promo/i.test(p.name))
      .map((p: any) => p.name);
    ...
    const seoVendor = await aiChat([
      {
        role: "system",
        content:
          "Write a 20-30 word SEO blurb for a food delivery vendor in South Africa. No emojis.",
      },
    ...
    const specialAdvice = await aiChat([
      {
        role: "system",
        content:
          "Given recent order timestamps (ISO), infer two best time windows to run promotions. Keep answer under 40 words.",
      },
    ...
    const daily = await aiChat([
      {
        role: "system",
        content:
          "Write a concise daily business summary for a vendor based on order count and revenue (ZAR). Keep under 40 words.",
      },
      {
        role: "user",
        content: `Orders: ${vendor.orders.length}, Revenue: R${(vendor.orders.reduce((s: number, o: any) => s + o.totalCents, 0) / 100).toFixed(0)}`,
      },
    ] as any);
    ...
    const altAdvice = withImgs
      .slice(0, 5)
      .map((p: any) => `Alt for ${p.name}: appetizing ${p.name} on a plate`);
