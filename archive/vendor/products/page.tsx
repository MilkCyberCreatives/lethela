  ...
  return (
    <main className="container py-8">
      ...
      <p className="text-white/70 mt-1">
        Create and manage your items. Toggle “Alcohol” for age-restricted products.
      </p>
      ...
      <form onSubmit={save} className="card-glass rounded-xl p-5 mt-6 grid gap-4 sm:grid-cols-2">
        ...
        <div className="sm:col-span-1">
          <label className="text-sm flex items-center gap-2">
            <input
              ...
              onChange={(e) => setForm((f) => ({ ...f, isAlcohol: e.target.checked }))}
            />
            Alcohol
          </label>
        </div>
        <div className="sm:col-span-1">
          ...
          <Input
            ...
            value={form.abv ?? ""}
            onChange={(e) =>
              setForm((f) => ({ ...f, abv: e.target.value ? Number(e.target.value) : null }))
            }
            className="bg-white text-black"
          />
        </div>
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 mt-6">
        {loading ? (
          Array.from({ length: 6 }).map((_, i) => (
            <div
              key={i}
              className="rounded-xl border border-white/10 bg-white/5 h-40 animate-pulse"
            />
          ))
        ) : items.length === 0 ? (
          ...
          items.map((p) => (
            <div key={p.id} className="rounded-xl border border-white/10 bg-white/[0.04] p-4">
              <div className="flex items-start justify-between gap-3">
                <div>
                  ...
                  <p className="text-xs text-white/60">
                    {p.isAlcohol ? "Alcohol • " : ""}R {(p.priceCents / 100).toFixed(2)}
                  </p>
                </div>
              ...
              {p.description && (
                <p className="text-sm text-white/70 mt-2 line-clamp-2">{p.description}</p>
              )}
            </div>
          ))
        )}
